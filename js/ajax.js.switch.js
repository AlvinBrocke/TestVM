					...
					if($(this).parents("form").length==0){
						...
						if(webSiteHref.indexOf(hrefDomen)!=-1 && followLinkTest && linkHref.indexOf("assets")==-1){
							if(linkHref!=currHref && linkHref.indexOf($(">a", currButton).attr("href"))==-1 && animationComplete && splash){
								if($(this).parents(customObject.classMenu)[0]){
									clickBool = true;
								}
								currHref = linkHref;
		...
		function changeActiveButton(){
			...
			if($(">a", prevButton).parents("li", customObject.classMenu).length!=0){
				if($(">a", prevButton).length!=0){
					...
					$(">a",prevButton).removeClass(customObject.activeClass);
				}
			}
			if(currButton && currButton!=""){
				if(clickBool){
					currButton = currButton.filter("li").find(".activeLink");
				}else{
					currButton = currButton.filter("li");
				}
				...
				xhr = $.ajax({
					...
					beforeSend:function(){
						...
						if(url.indexOf("search.php")==-1){
							$("a", customObject.classMenu).each(function(){
								if($(this).parents(customObject.contentHolder).length==0){
									if(this.href==url){
										currButton=$(this).parent();
									}
								}
							});
						}else{
